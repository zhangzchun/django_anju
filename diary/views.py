    ...
    if request.method == "GET":
        ...
        try:
            ...
            sql = "SELECT d.id,u.nickname , ui.icon user_icon , d.diary_title ,d.publish_date,d.area, s.`name` style , \
                    ...
                    search_renovationtype rt on u.userIcon_id=ui.id  && d.user_id=u.id && d.style_id=s.id && d.renovationType_id=rt.id\
...
def diaryDetailContent(request):
    if request.method == "GET":
        diary_id=request.GET.get("diary_id")
        try:
            dairy_content = models.diaryContent.objects.filter(diary_id=diary_id).values("id","stage", "publish_date", "diary_content")
...
def diaryUserIcon(request):
    if request.method == "GET":
        try:
            dairy_content = models.diaryInfo.objects.all().values("id","user__userIcon__icon").order_by("id")[0:12]
